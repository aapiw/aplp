if Rails.env.production?
	...
	if Admin.where(role:"admin").count < 1
		adm = Admin.new
		adm.username = ENV["BIPA_ADMIN"]
		adm.password = ENV["ADMIN_PASSWORD"]
		adm.password_confirmation = ENV["ADMIN_PASSWORD"]
	...
	if Landing.count < 1
		Landing.find_or_create_by(title:"Selamat datang di aplikasi BIPA 2018 <br> Atdikud BPKLN Kemendikbud RI")
	end
