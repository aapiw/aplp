...
var ready = function() {
  ...
  $("#scoring [type=number]").on('keyup mousewheel keypress blur change', function(e) {
    var sum = [];
    $("#scoring [type=number]").each(function(index, el) {
      sum.push($(el).val())
    });
    var $input =  $(".total-nilai");
    if (sumAll(sum) > 100) {
      $input.parents(".form-line").addClass('error');
    }else{
      $input.parents(".form-line").removeClass('error');
    }
    $input.val(sumAll(sum));
    sum = [];
  });
